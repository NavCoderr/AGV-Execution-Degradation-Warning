...
Route provenance is retained explicitly so that recorded and reconstructed route context remain distinguishable.
...
The aggregate values below use the `complete_test_session` rows in `outputs/scientific_experiments/05_gap_robustness.csv`.
...
This script generates the shared tabular LOSO outputs, communication-gap stress test, latency benchmark, and representation-comparison outputs.
